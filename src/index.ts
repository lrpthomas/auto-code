...
import { OrchestratorAlpha } from './orchestrator';
...
`);
  ...
  const orchestrator = new OrchestratorAlpha(DEFAULT_CONFIG);
...
async function runDemo(orchestrator: OrchestratorAlpha) {
  const demoApps = [
...
async function startAPIServer(orchestrator: OrchestratorAlpha) {
  ...
  app.get('/apps/:id/files', async (_req: any, res: any) => {
...
export { OrchestratorAlpha, DEFAULT_CONFIG };
export * from './types';
