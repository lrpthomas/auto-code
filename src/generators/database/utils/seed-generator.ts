import { DatabaseGenerator, SchemaTemplate, TableConfig, FieldConfig } from '../index';
...
export class SeedGenerator {
  ...
  private initializeFaker(): void {
    ...
    this.faker = {
      ...
      datatype: {
        number: (options?: { min?: number; max?: number }) => Math.floor(Math.random() * 100),
  ...
  async generateSeeds(template: SchemaTemplate, configs: SeedConfig[]): Promise<string> {
  ...
  private generateTableData(table: TableConfig, config: SeedConfig): any[] {
    const data: any[] = [];
    ...
    for (let i = 0; i < config.count; i++) {
      ...
      for (const field of table.fields) {
        ...
        if (this.isAutoGeneratedField(field)) continue;
  ...
  private isAutoGeneratedField(field: FieldConfig): boolean {
  ...
  private generateFieldValue(field: FieldConfig, index: number, config: SeedConfig): any {
  ...
  private generateRelationValue(relation: RelationSeedConfig, index: number): any {
  ...
  private generateStringValue(field: FieldConfig): string {
  ...
  private generateTextValue(field: FieldConfig): string {
    const fieldName = field.name.toLowerCase();
  ...
  private generateIntegerValue(field: FieldConfig): number {
    const fieldName = field.name.toLowerCase();
  ...
  private generateDecimalValue(field: FieldConfig): number {
    const fieldName = field.name.toLowerCase();
  ...
  private generateDateValue(field: FieldConfig): Date {
    const fieldName = field.name.toLowerCase();
  ...
  private generateJsonValue(field: FieldConfig): Record<string, any> {
    const fieldName = field.name.toLowerCase();
  ...
  private generateRelationConfigs(table: TableConfig): RelationSeedConfig[] {
    const relations: RelationSeedConfig[] = [];
  ...
  async generateFactoryFile(template: SchemaTemplate): Promise<string> {
  ...
  private generatePostgresFactory(template: SchemaTemplate): string {
  ...
  private generateMongoFactory(template: SchemaTemplate): string {
    let factory = `// Database Factory for ${template.name}\n`;
    factory += `import { faker } from '@faker-js/faker';\n`;
  ...
  private generateFactoryFieldValue(field: FieldConfig): string {
    switch (field.type) {
      case 'string':
  ...
  private generateFactoryStringValue(field: FieldConfig): string {
    const fieldName = field.name.toLowerCase();
