...
export class PerformanceOptimizer {
  ...
  private analyzeTablePerformance(table: TableConfig): OptimizationSuggestion[] {
    const suggestions: OptimizationSuggestion[] = [];
    ...
    if (table.relationships) {
      for (const rel of table.relationships) {
        const hasIndex = table.indexes?.some(index =>
          index.fields.includes(rel.foreignKey)
        );
  ...
  private analyzeTextSearchOptimization(table: TableConfig): OptimizationSuggestion[] {
    const suggestions: OptimizationSuggestion[] = [];
    const textFields = table.fields.filter(f => 
      ['string', 'text'].includes(f.type) && 
      ['title', 'name', 'description', 'content'].some(keyword => 
  ...
  private analyzeDateRangeOptimization(table: TableConfig): OptimizationSuggestion[] {
    const suggestions: OptimizationSuggestion[] = [];
    const dateFields = table.fields.filter(f => 
      ['date', 'datetime', 'timestamp'].includes(f.type)
    );

    for (const dateField of dateFields) {
      const hasIndex = table.indexes?.some(index => 
        index.fields.includes(dateField.name)
      );
  ...
  private analyzeCompositeIndexes(table: TableConfig): OptimizationSuggestion[] {
    const suggestions: OptimizationSuggestion[] = [];
    ...
    if (statusField && createdField) {
      const hasCompositeIndex = table.indexes?.some(index => 
        index.fields.includes(statusField.name) && index.fields.includes(createdField.name)
      );
  ...
  private analyzeRelationshipPerformance(): OptimizationSuggestion[] {
    const suggestions: OptimizationSuggestion[] = [];
  ...
  private analyzeIndexes(): IndexAnalysis {
      ...
      for (let i = 0; i < indexes.length; i++) {
        for (let j = i + 1; j < indexes.length; j++) {
          ...
          if (index1.fields.length < index2.fields.length) {
            const isPrefix = index1.fields.every((field, idx) => 
              index2.fields[idx] === field
            );
            if (isPrefix) {
  ...
  private identifyQueryPatterns(): QueryPattern[] {
  ...
  private calculatePerformanceScore(suggestions: OptimizationSuggestion[]): number {
    let score = 100;
  ...
  generateOptimizedSchema(): SchemaTemplate {
    const optimizedTemplate: SchemaTemplate = JSON.parse(JSON.stringify(this.template));
  ...
  private applyIndexOptimization(template: SchemaTemplate, suggestion: OptimizationSuggestion): void {
    const table = template.tables.find(t => t.name === suggestion.table);
    if (!table || !suggestion.field) return;
  ...
  private formatSuggestion(suggestion: OptimizationSuggestion): string {
