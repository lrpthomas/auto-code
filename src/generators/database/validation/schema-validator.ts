import { SchemaTemplate, TableConfig, FieldConfig, RelationshipConfig, ConstraintConfig } from '../index';
...
export class SchemaValidator {
    ...
    return [
      {
        ...
        validate: (value: any, field: FieldConfig) => {
  ...
  private validateSchemaStructure(template: SchemaTemplate, errors: ValidationError[], warnings: ValidationWarning[]): void {
  ...
  private validateTable(table: TableConfig, errors: ValidationError[], warnings: ValidationWarning[]): void {
    // Check for primary key
    const hasPrimaryKey = table.fields.some(field => 
      field.constraints?.some(constraint => constraint.type === 'primary')
    );
    ...
    const hasCreatedAt = table.fields.some(f => 
      f.name.toLowerCase().includes('created') && ['date', 'datetime', 'timestamp'].includes(f.type)
    );
    const hasUpdatedAt = table.fields.some(f => 
      f.name.toLowerCase().includes('updated') && ['date', 'datetime', 'timestamp'].includes(f.type)
    );
  ...
  private validateField(field: FieldConfig, table: TableConfig, errors: ValidationError[], warnings: ValidationWarning[]): void {
  ...
  private validateConstraints(field: FieldConfig, table: TableConfig, errors: ValidationError[], warnings: ValidationWarning[]): void {
  ...
  private validateFieldTypeSpecific(field: FieldConfig, table: TableConfig, errors: ValidationError[], warnings: ValidationWarning[]): void {
    switch (field.type) {
      case 'string':
  ...
  private validateRelationships(table: TableConfig, allTables: TableConfig[], errors: ValidationError[], warnings: ValidationWarning[]): void {
    for (const relationship of table.relationships!) {
      ...
      const hasIndex = table.indexes?.some(index => 
        index.fields.includes(relationship.foreignKey)
      );
  ...
  generateValidationSchema(template: SchemaTemplate, format: 'joi' | 'yup' | 'zod' = 'zod'): string {
    switch (format) {
      case 'joi':
  ...
  private generateZodSchema(template: SchemaTemplate): string {
    ...
    for (const table of template.tables) {
      ...
      const omitFields = [];
      if (table.fields.some(f => f.name === 'id')) omitFields.push('id');
      if (table.fields.some(f => f.name.includes('created'))) omitFields.push('createdAt');
      if (table.fields.some(f => f.name.includes('updated'))) omitFields.push('updatedAt');
  ...
  private generateZodFieldValidation(field: FieldConfig): string {
    let validation = '';
  ...
  private generateJoiSchema(template: SchemaTemplate): string {
  ...
  private generateJoiFieldValidation(field: FieldConfig): string {
    let validation = '';
  ...
  private generateYupSchema(template: SchemaTemplate): string {
  ...
  private generateYupFieldValidation(field: FieldConfig): string {
    let validation = '';
