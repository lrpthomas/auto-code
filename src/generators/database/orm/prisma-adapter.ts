...
export class PrismaAdapter {
  ...
  private generateModel(table: TableConfig, allTables: TableConfig[]): string {
  ...
  private generateField(field: FieldConfig): string {
  ...
  private generateRelationship(table: TableConfig, rel: RelationshipConfig, allTables: TableConfig[]): string {
  ...
  private generateIndex(index: IndexConfig): string {
  ...
  private mapFieldToPrisma(field: FieldConfig): string {
    switch (field.type) {
      case 'string':
        return 'String';
      case 'text':
...
`;
  ...
  generateSeedFile(template: SchemaTemplate): string {
    ...
    for (const table of template.tables) {
      ...
      for (const field of table.fields) {
        if (field.constraints?.some(c => c.type === 'primary')) continue;
        if (field.name.toLowerCase().includes('created') || field.name.toLowerCase().includes('updated')) continue;
  ...
  private generateSeedValue(field: FieldConfig, indexVar: string): string {
