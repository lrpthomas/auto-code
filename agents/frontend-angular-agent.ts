...
export default class FrontendAngularAgent implements Agent {
  ...
  private async generateAngularApp(requirements: any): Promise<Record<string, string>> {
  ...
  private generateAngularJson(requirements: any): string {
    const appName = requirements.description?.split(' ').slice(0, 3).join('-').toLowerCase().replace(/[^a-z0-9-]/g, '') || 'angular-app';
  ...
  private generateAppTsConfig(): string {
    return JSON.stringify({
...
export class AppComponent {
  ...
  private generateAppTemplate(requirements: any): string {
  ...
  private generateAppStyles(): string {
...
@media (max-width: 768px) {
  ...
  private generateAppConfig(requirements: any): string {
...
export const appConfig: ApplicationConfig = {
  ...
  private generateRoutes(requirements: any): string {
    const hasAuth = requirements.features?.some((f: string) => f.toLowerCase().includes('auth'));
...
export class NotFoundComponent {}`;
  ...
  private generateAuthComponents(requirements: any, files: Record<string, string>): void {
    // Login component
...
export class ApiService {
  ...
  private generateAuthStore(files: Record<string, string>): void {
    // Auth actions
...
export class AuthEffects {
  ...
  private generateAuthGuards(files: Record<string, string>): void {
    // Auth guard
...
export const guestGuard: CanActivateFn = () => {
  ...
  private generateAuthInterceptor(files: Record<string, string>): void {
