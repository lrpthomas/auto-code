...
export default class FrontendVueAgent implements Agent {
  ...
  private async generateVueApp(requirements: any): Promise<Record<string, string>> {
...
const authStore = useAuthStore()
  ...
  private generateComposables(requirements: any, files: Record<string, string>): void {
    // API composable
...
export function useLocalStorage<T>(key: string, defaultValue: T) {
  ...
  private generateStore(requirements: any, files: Record<string, string>): void {
    const hasAuth = requirements.features?.some((f: string) => f.toLowerCase().includes('auth'));
...
export const useNotificationStore = defineStore('notifications', () => {
  ...
  private generateRouter(requirements: any): string {
    const hasAuth = requirements.features?.some((f: string) => f.toLowerCase().includes('auth'));
