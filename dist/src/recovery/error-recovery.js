...
exports.SystemHealthMonitor = exports.ResilientAgent = exports.ErrorRecoveryManager = void 0;
...
class ErrorRecoveryManager extends events_1.EventEmitter {
    ...
    async handleError(agent, task, error, attempt = 1) {
        const context = {
            agentId: agent.id,
            taskId: task.id,
            error,
    ...
    getErrorHistory(agentId, limit = 100) {
    ...
    initializeRecoveryStrategies() {
        ...
        this.registerStrategy({
            ...
            execute: async (context, agent) => {
                const delay = Math.min(this.config.retry.baseDelayMs * Math.pow(this.config.retry.backoffMultiplier, context.attempt - 1), this.config.retry.maxDelayMs);
        ...
        this.registerStrategy({
            ...
            priority: 2,
            canHandle: (context) => context.error.message.includes('resource') ||
        ...
        this.registerStrategy({
            ...
            priority: 3,
            canHandle: (context) => context.error.message.includes('timeout') ||
        ...
        this.registerStrategy({
            ...
            priority: 4,
            canHandle: (context) => context.error.message.includes('dependency') ||
...
class ResilientAgent {
    baseAgent;
    recoveryManager;
    constructor(baseAgent, recoveryManager) {
        ...
        while (attempt <= 3) { // Max attempts
            try {
                ...
                if (attempt === 3) {
                    ...
                    return await this.recoveryManager.handleError(this.baseAgent, task, error instanceof Error ? error : new Error(String(error)), attempt);
                }
                attempt++;
...
// Health Monitor
class SystemHealthMonitor extends events_1.EventEmitter {
    checkIntervalMs;
    agents = new Map();
    ...
    getSystemHealth() {
        ...
        const statusCounts = Array.from(this.healthStatus.values()).reduce((acc, status) => {
