...
class BackupManager {
    ...
    async createBackup() {
        ...
        try {
            ...
            else if (this.generator instanceof mongodb_1.MongoDBGenerator) {
    ...
    async createPostgresBackup(filepath) {
...
`;
    ...
    async createMongoBackup(filepath) {
        const dbName = this.generator['config'].name;
        let script = `#!/bin/bash
...
`;
    ...
    async createPostgresRestore(filepath, options) {
...
`;
    ...
    async createMongoRestore(filepath, options) {
        const dbName = options.targetDatabase || this.generator['config'].name;
        return `#!/bin/bash
...
const { ${this.generator instanceof postgresql_1.PostgreSQLGenerator ? 'PostgreSQLGenerator' : 'MongoDBGenerator'} } = require('../${this.generator instanceof postgresql_1.PostgreSQLGenerator ? 'postgresql' : 'mongodb'}');

const generator = new ${this.generator instanceof postgresql_1.PostgreSQLGenerator ? 'PostgreSQLGenerator' : 'MongoDBGenerator'}({ name: '${this.generator['config'].name}' });
...
`;
    ...
    generateBackupFilename(timestamp) {
