...
phases:
  ...
  "containerDefinitions": [
    ...
    async generateDeployment(app, requirements) {
    ...
    generateDockerConfig(requirements) {
        const files = {};
    ...
    generateKubernetesConfig(requirements) {
        const files = {};
        const appName = this.getAppName(requirements);
        const hasBackend = !!requirements.techStack.backend;
        const hasFrontend = !!requirements.techStack.frontend;
...
---`;
        ...
        if (hasFrontend) {
            files['k8s/frontend-deployment.yaml'] = this.renderTemplate(this.deploymentTemplates.get('kubernetes-deployment').deployment, {
        ...
        if (hasBackend) {
            files['k8s/backend-deployment.yaml'] = this.renderTemplate(this.deploymentTemplates.get('kubernetes-deployment').deployment, {
        ...
        // Ingress
        files['k8s/ingress.yaml'] = this.renderTemplate(this.deploymentTemplates.get('kubernetes-deployment').ingress, {
            appName,
...
spec:
  ...
  - type: Resource
    ...
    generateVercelConfig(requirements) {
        const files = {};
        const appName = this.getAppName(requirements);
        files['vercel.json'] = this.renderTemplate(this.deploymentTemplates.get('vercel-config').config, {
            appName,
            envVars: this.getEnvironmentVars(requirements, 'frontend'),
            buildEnvVars: [
    ...
    generateAWSConfig(requirements) {
        const files = {};
        const appName = this.getAppName(requirements);
        const hasBackend = !!requirements.techStack.backend;
        if (hasBackend) {
            files['buildspec.yml'] = this.deploymentTemplates.get('aws-config').buildspec;
            files['task-definition.json'] = this.renderTemplate(this.deploymentTemplates.get('aws-config').taskDefinition, {
                appName,
                cpu: '512',
                memory: '1024',
                containerPort: 3000,
...
${volumes}`;
    ...
    generateDockerComposeProd(requirements) {
        return `version: '3.8'
...
.env.test`;
    ...
    generateK8sConfigMap(requirements) {
        const appName = this.getAppName(requirements);
...
${configData}`;
    ...
    generateK8sSecrets(requirements) {
        const appName = this.getAppName(requirements);
        return `apiVersion: v1
kind: Secret
metadata:
...
data:
  ...
  database-password: cGFzc3dvcmQ=`;
    ...
    generateCloudFormationTemplate(requirements) {
        const appName = this.getAppName(requirements);
...
Outputs:
  LoadBalancerDNS:
    ...
    generateDeploymentInstructions(platform, config, requirements) {
        const appName = this.getAppName(requirements);
        switch (platform) {
            case 'docker':
    ...
    generateDockerInstructions(config, appName) {
...
- Database: Check logs for connection status`;
    ...
    generateK8sInstructions(config, appName) {
...
\`\`\``;
    ...
    generateVercelInstructions(config, appName) {
...
- Enable preview deployments for pull requests`;
    ...
    generateAWSInstructions(config, appName) {
...
\`\`\``;
    ...
    estimateDeploymentTime(platform, requirements) {
        const baseTime = {
        ...
        if (requirements.techStack.database)
            time += 60;
        if (requirements.techStack.frontend && requirements.techStack.backend)
            time += 60;
        if (requirements.features.length > 5)
            time += 30;
        return time; // seconds
    }
    getAppName(requirements) {
    ...
    getRequiredPorts(requirements) {
        const ports = [];
        if (requirements.techStack.frontend)
            ports.push(3000, 80);
        if (requirements.techStack.backend)
            ports.push(3001, 8080);
    ...
    getEnvironmentVars(requirements, type) {
        const vars = [];
        if (type === 'frontend') {
