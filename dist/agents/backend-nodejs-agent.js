...
class BackendNodejsAgent {
    ...
    async generateNodeApp(requirements) {
        const files = {};
        // Package.json
    ...
    generateEnvExample(requirements) {
        ...
        if (requirements.techStack.database === 'postgresql') {
            vars.push('DATABASE_URL=postgresql://username:password@localhost:5432/database', 'DB_HOST=localhost', 'DB_PORT=5432', 'DB_USER=username', 'DB_PASSWORD=password', 'DB_NAME=database');
        }
        else if (requirements.techStack.database === 'mongodb') {
...
export function createApp(): express.Application {
  ...
  return app;
    ...
    generateDatabaseConfig(requirements) {
...
export async function closeDatabase(): Promise<void> {
  if (db) {
    ...
    generateMiddleware(requirements, files) {
...
export function validateRequest(schema: {
  ...
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    generateRoutes(requirements, files) {
        const hasAuth = requirements.features.some((f) => f.toLowerCase().includes('auth'));
        // Main routes index
...
export { router as authRoutes };`;
    ...
    generateControllers(requirements, files) {
        const hasAuth = requirements.features.some((f) => f.toLowerCase().includes('auth'));
        // Health controller
...
exports.default = BackendNodejsAgent;
//# sourceMappingURL=backend-nodejs-agent.js.map